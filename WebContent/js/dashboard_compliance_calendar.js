...
function calendar_set_doday() {
	...
	if (typeof(calendar_element.value) != 'undefined') {
		calendar_element.value = toDay;getChart(y+'-'+m+'-'+d, timeArea);//키워드 현황 차트
		getImportantIssue(y+'-'+m+'-'+d, timeArea);//주요이슈
		getKeyCollectList(y+'-'+m+'-'+d, timeArea);//키워드 수집	
		getRegIssueList(y+'-'+m+'-'+d, timeArea);//등록이슈
...
function calendar_onclick(d, m, y) {
	...
	if(searchMode == 'keyConditionSDay'){
		// 달력으로 날짜를 변경했을 경우, TimeArea는 무시되고 선택된 날짜를 기준으로 검색한다.
		changeAllButtonOff();
		
		if(document.getElementById('tarEdate').value.split(" ")[0].replace(/\./g,'') < document.getElementById('tarSdate').value.split(" ")[0].replace(/\./g,'')){
			alert('시작일을 종료일 앞으로 선택해주세요');
		}
		
		var eDate = document.getElementById('tarEdate').value.split(" ")[0].replace(/\./g,'-');
		getChart(eDate, 6, y+'-'+m+'-'+d);
		getImportantIssue(eDate, 6, y+'-'+m+'-'+d);
		getKeyCollectList(eDate, 6, y+'-'+m+'-'+d);
		getRegIssueList(eDate, 6, y+'-'+m+'-'+d);
	}else if(searchMode == 'keyConditionEDay'){
		changeAllButtonOff();
		// 달력으로 날짜를 변경했을 경우, TimeArea는 무시되고 선택된 날짜를 기준으로 검색한다.
		if(document.getElementById('tarEdate').value.split(" ")[0].replace(/\./g,'') < document.getElementById('tarSdate').value.split(" ")[0].replace(/\./g,'')){
			alert('종료일을 시작일 앞으로 선택해주세요');
		}else{
		var sDate = document.getElementById('tarSdate').value.split(" ")[0].replace(/\./g,'-');
		getChart(y+'-'+m+'-'+d, 6, sDate);
		getImportantIssue(y+'-'+m+'-'+d, 6, sDate);
		getKeyCollectList(y+'-'+m+'-'+d, 6, sDate);
		getRegIssueList(y+'-'+m+'-'+d, 6, sDate);
		}
	}else if(searchMode == 'stackedSDay'){
		//stackedSDay
		if(document.getElementById('stackedConditionDay').value.split(" ")[0].replace(/\./g,'') < document.getElementById('stackedSDay').value.split(" ")[0].replace(/\./g,'')){
			alert('시작일을 종료일 뒤로 선택해주세요');
		}else{
		var eDate = document.getElementById('stackedConditionDay').value.split(" ")[0].replace(/\./g,'-');
		//var eDate = document.getElementById('stackedConditionDay').value;
		getStackedBarChart(eDate, 5, y+'-'+m+'-'+d);
		getColumnChart(eDate, 5, y+'-'+m+'-'+d);
		changeAllButtonOff2();
		}
	}else if(searchMode == 'stackedConditionDay'){
		//stackedConditionDay
		if(document.getElementById('stackedConditionDay').value.split(" ")[0].replace(/\./g,'') < document.getElementById('stackedSDay').value.split(" ")[0].replace(/\./g,'')){
			alert('종료일을 시작일 앞으로 선택해주세요');
		}else{
		var sDate = document.getElementById('stackedSDay').value.split(" ")[0].replace(/\./g,'-');
		getStackedBarChart(y+'-'+m+'-'+d, 5, sDate);
		getColumnChart(y+'-'+m+'-'+d, 5, sDate);
		changeAllButtonOff2();
		changeStackedSdate(y+'-'+m+'-'+d, 5, sDate);
		}
	}else if(searchMode == 'emergency'){
	}
}
