...
function calendar_onclick(d, m, y) {
	...
	if(d < 10){d='0'+d;}
	if(searchMode == 'today'){
		getTodayConditionChart_IT(y+'-'+m+'-'+d, 1);
		getTodayConditionChart_xEV_ESS(y+'-'+m+'-'+d, 1);	
		showTimeArea(y+'-'+m+'-'+d);
	}else if(searchMode == 'issue'){
		getIssueConditionChart(y+'-'+m+'-'+d, 'week');
		getIssueConditionChart(y+'-'+m+'-'+d, 'month');
		getIssueConditionChart(y+'-'+m+'-'+d, 'quarter');
	}
}
